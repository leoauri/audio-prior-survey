...
def get_args():
    parser = argparse.ArgumentParser(
        "DAWP", description="Train and evaluate Deep Audio Waveform Prior."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-e", "--epochs", type=int, default=5000, help="Number of epochs"
    )
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument(
        "--show_every", type=int, default=50, help="Epochs between plots"
    )
    ...
    parser.add_argument(
        "--show_estimates_vs_noise",
        "--show_overfit",
        action="store_true",
        default=False,
        dest="show_estimates_vs_noise",
        help="In metrics plot add graph of metric(estimate,noisy)",
    )
    parser.add_argument("--source", required=True, type=Path, help="Path to inputs")
    parser.add_argument(
        "--output_dir", type=Path, default="./evals", help="Path for experiments output"
    )
    ...
    parser.add_argument(
        "--zero_rate", type=float, default=0.01, help="Proportion of samples to zero"
    )

    parser.add_argument(
        "--noise_class", type=str, default="GAUSSIAN", choices=["GAUSSIAN", "UNIFORM"]
    )
    ...
    parser.add_argument(
        "--attention_layers", type=int, default=0, help="Number of attention layers"
    )
    parser.add_argument(
        "--attention_heads", type=int, default=2, help="Number of attention heads"
    )
    parser.add_argument(
        "--depth",
        type=int,
    ...
    parser.add_argument(
        "--ncp_neurons", type=int, default=5, help="Number of neurons in NCP model"
    )

    parser.add_argument(
        "--sashimi_channels", type=int, default=8, help="SaShiMi architecture channels"
    )
    ...
    if epoch % args.show_every == 0:
        if args.save_outputs:
            ...
            _plot_wavs(
                wavs,
                sr=args.samplerate,
                prefix=str(basedir / "img_wavs" / f"{epoch:05d}"),
            )
            _plot_wav_to_stft(
                wavs,
                sr=args.samplerate,
                prefix=str(basedir / "img_mel" / f"{epoch:05d}"),
            )
            wavfile.write(
                str(basedir / "wavs" / f"epoch_{epoch:05d}_output.wav"),
        ...
        for metric in metrics_tracker:
            _plot_metric(
                metric, basedir, show_estimates_vs_noise=args.show_estimates_vs_noise
            )
...
def save_repetition_results(metric_tracker, loss, samplerate, basedir):
    ...
    for metric in metric_tracker:
        wavfile.write(
            str(basedir / f"{metric}_best.wav"), rate=samplerate, data=metric.get_wav()
        )
        best[str(metric)] = {
            "value": f"{metric.best_clean:.4f}",
...
def save_experiment_results(experiment_path, logger, repetitions_to_ignore=()):
    ...
    with open(str(experiment_path / "metrics_best_stat.json"), "w") as f:
        vals = {
            metric: np.array(
                [results[metric][rep]["value"] for rep in results[metric]]
            ).astype(np.float)
            for metric in results
        }
        ref = {
            metric: np.array(
                [results[metric][rep]["reference"] for rep in results[metric]]
            ).astype(np.float)
            for metric in results
        }
        stat = {
            metric: {
                "mean": f"{vals[metric].mean():.4f}",
                "std": f"{vals[metric].std():.4f}",
                "delta mean": f"{(vals[metric] - ref[metric]).mean():.4f}",
                "repetitions": len(vals[metric]),
            }
            for metric in vals
        }
