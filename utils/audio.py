...
def filter_quiet(tracks, thresh=0.1):
...
def process_audio(args, audio, audio_sr, logger, chunk_index=None):
    ...
    if chunk_index is None:
        audio_chunks, audio_chunks_indices = filter_quiet(
            audio_chunks, args.quiet_thresh
        )
        if not audio_chunks.any():
            return None, None
...
def random_replace(clean_audio, replacement, proportion):
    ...
    if noisy_audio.shape == replacement.shape:
        ...
        noisy_audio[indexes] = np.broadcast_to(replacement, noisy_audio.shape).copy()[
            indexes
        ]
    return noisy_audio
...
def get_clean_n_noisy(paths, args, logger):
    ...
    if noisy_path:
        try:
            ...
            return None
        assert (
            sr_noisy == sr_clean == args.samplerate
        ), f"Samplesrates doesnt match! Required samplerate: {args.samplerate}. Clean audio samplerate: {sr_clean}. Noisy audio samplerate: {sr_noisy}."
        assert (
            clean.shape[-1] == noisy.shape[-1]
        ), f"Mismatch length! clean audio :{clean.shape[-1]}, noisy audio: {noisy.shape}"
        ...
        f(
            f"Loaded noisy audio with SNR {loaded_snr:.2f}. Specified SNR in args is {args.snr:.2f}"
        )
    ...
    return (
        torch.from_numpy(clean_audio).unsqueeze(0).unsqueeze(0).to(args.device),
        torch.from_numpy(noisy_audio).unsqueeze(0).unsqueeze(0).to(args.device),
        clean_path,
    )
